...
class Experiment:
    ...
    def run_epoch(self, data_loader, sample_type, trainable=False):
        ...
        for i, batch in enumerate(data_loader):
            ...
            if np.isnan(loss_sum/self.step):
                print('loss :', loss)
    ...
    def get_score(self):
        ...
        wP /= len(self.yp_counter)
        #print('y em :', self.y_em_counter, len(self.y_em_counter))
        #print(self.yp_counter, wP, len(self.yp_counter))
        for y, cnt in self.yt_counter.items():
            wR += self.y_em_counter[y] / cnt
        wR /= len(self.yt_counter)
        #print(self.yt_counter, wR, len(self.yt_counter))
        #sys.exit()
        """
        # metric used by Wang
        wP = 0
